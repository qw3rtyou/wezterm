return {
   ...
   ssh_domains = {
      ...
      {
         name = 'kali-local',
         -- 같은 LAN(172.30.1.0/24)이므로 공유기를 거치지 않고 직접 붙는다.
         -- 공인 IP(211.250.216.249)로 붙으면 헤어핀 NAT 경로가 되어 공유기 NAT
         -- 세션 테이블에 잡히고, 정확히 1시간 뒤 끊겼다(2026-09-03 실측:
         -- last 기록 14:57:48-15:58:05 = 01:00, 서버가 본 peer 는 공유기 주소).
         -- 서버 IP 는 DHCP 이므로 공유기에 고정 할당을 걸어두는 편이 안전하다.
         -- 외부망에서 붙어야 할 때는 공인 IP 211.250.216.249 를 쓴다.
         remote_address = '172.30.1.76',
         username = 'foo1',
